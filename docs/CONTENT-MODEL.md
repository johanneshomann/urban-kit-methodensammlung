...
- **`PlatformSettings`** (admin-only): brand + text colors, branding uploads (admin
  ...
  rich text), and the Über page content (localized richText, rendered at `/ueber`), and the footer
  sponsors (array of logo upload + name + optional URL; rendered as the
  "Gefördert durch" band below the footer's legal row — name via tooltip only —
  and as a card grid with visible names on `/ueber`). There is no contact form — the Kontakt page is mailto-only; SMTP env
