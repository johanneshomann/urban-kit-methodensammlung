...
export async function generateMetadata({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'barrierefreiheit' })
...
export default async function BarrierefreiheitPage({ params }: Props) {
  ...
  let content = settings.barrierefreiheit
  if (locale !== 'de' && !hasRichTextContent(content)) {
    content = (await payload.findGlobal({ slug: 'legal' as any, locale: 'de' })).barrierefreiheit
  }
  ...
  return (
    <div>
      ...
      >
        ...
        <a href="#barrierefreiheit-content">
          <ChevronDown
